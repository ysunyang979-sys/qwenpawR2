 ...
 * Self-registers into the left sidebar under 'Plugins' as 'Cloudflare R2'.
 */
(function () {
  var QwenPaw = window.QwenPaw;
  if (!QwenPaw || !QwenPaw.host || !QwenPaw.registerRoutes) {
    console.warn("[cloudflare-r2] QwenPaw host not ready, delaying registration...");
    return;
  }
  ...
  function R2Explorer() {
    var [currentPath, setCurrentPath] = React.useState("");
    var [entries, setEntries] = React.useState([]);
    var [loading, setLoading] = React.useState(false);
    var [status, setStatus] = React.useState({ configured: false, connected: false, bucket: "", account_id: "" });
    
    // Preview modal
    var [previewModal, setPreviewModal] = React.useState({ visible: false, title: "", content: "", loading: false });
    
    // Config modal
    var [configModal, setConfigModal] = React.useState({
      visible: false,
      account_id: "",
      access_key_id: "",
      secret_access_key: "",
    ...
    // Upload modal
    var [uploadModal, setUploadModal] = React.useState({ visible: false, fileName: "", fileContent: "", uploading: false });
    ...
    var fetchStatus = function () {
      apiFetch("/r2/status").then(function (res) {
        ...
        console.error("Fetch R2 status failed:", err);
      });
    };
    ...
    return h("div", { style: { padding: "20px 24px", height: "100%", overflowY: "auto", background: "#f8fafc" } },
      ...
      h(Modal, {
        ...
        h("div", { style: { display: "flex", flexDirection: "column", gap: 14, paddingTop: 10 } },
          ...
          h("div", null,
            h("div", { style: { marginBottom: 4, fontWeight: "bold" } }, "文件名称 (例如 test.txt 或 doc/readme.md):"),
            h(Input, {
          ...
          h("div", null,
            h("div", { style: { marginBottom: 4, fontWeight: "bold" } }, "文件内容 (文本/JSON/代码):"),
            h(Input.TextArea, {
              rows: 8,
  ...
  // Self-register route into Sidebar Plugins group!
  QwenPaw.registerRoutes("cloudflare-r2-tool", [
    {
      path: "/cloudflare-r2",
  ...
  console.info("[cloudflare-r2] Registered route /cloudflare-r2 into QwenPaw sidebar");
})();
