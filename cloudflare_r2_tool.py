# -*- coding: utf-8 -*-
"""Cloudflare R2 Tool Implementation."""
...
def _get_service():
    from qwenpaw.services.r2_storage import get_r2_service
    return get_r2_service()


async def list_r2_files(prefix: str = "", limit: int = 50) -> ToolChunk:
    """List files and folders in the Cloudflare R2 bucket.
    ...
    try:
        ...
        if not entries:
            return _text_chunk(f"Cloudflare R2 路径 '{prefix}' 下未找到任何文件。")
        
        lines = [f"=== Cloudflare R2 路径 [{prefix or '/'}] 文件列表 (共 {len(entries)} 项) ==="]
        for e in entries:
...
async def read_r2_file(file_path: str, max_bytes: int = 50000) -> ToolChunk:
    """Read content of a text or code file from Cloudflare R2.

    Args:
        file_path (str): The relative path of the file in R2 (e.g. '文档/readme.txt').
...
async def upload_r2_file(file_path: str, content: str) -> ToolChunk:
    """Upload text or code content directly to Cloudflare R2.
...
async def delete_r2_file(file_path: str) -> ToolChunk:
    ...
    try:
        ...
        return _text_chunk(f"删除失败: 文件不存在或删除操作未完成", success=False)
    except Exception as exc:
...
async def get_r2_status() -> ToolChunk:
    ...
    try:
        ...
        if st.get("error"):
            status_lines.append(f"错误信息: {st.get('error')}")
        return _text_chunk("\n".join(status_lines))
    except Exception as exc:
        return _text_chunk(f"获取 R2 状态失败: {str(exc)}", success=False)
